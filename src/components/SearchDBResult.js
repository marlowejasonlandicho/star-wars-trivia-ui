...
function SearchDBResult({ searchPeopleResult }) {
  if (
    searchPeopleResult?.data?.searchPeopleFromDB?.people === undefined ||
    searchPeopleResult?.data?.searchPeopleFromDB?.people.length == 0
  ) {
    return <div>No Data</div>;
  } else {
    return (
      <div>
        {searchPeopleResult?.data?.searchPeopleFromDB?.people.map((people) => (
          <div>
            <h1>{people.name}</h1>
            <h2>Films</h2>
